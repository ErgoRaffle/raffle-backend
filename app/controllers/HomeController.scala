...
class HomeController @Inject()(adaptor: Adaptor, explorer: Explorer, client: Client, val controllerComponents: ControllerComponents, utils: Utils) extends BaseController
  ...
  def addRaffle(name: String, description: String, deadlineHeight: Long, organizerAddr: String, charityAddr: String, minToRaise: Long) = Action {
    try {
...
//      }
//      )
      val txId = adaptor.addRaffle(name, description, deadlineHeight, organizerAddr, charityAddr, minToRaise)
      Ok(
        s""""{
           |  txId : "$txId"
           |}""".stripMargin
